...
namespace cs296
{
  ...
  /** \brief
   * The settings_t struct describes all the settings which control the simulation of the Rube-Goldberg machine like the number of velocity iterations, position iterations. 
   
   Some of these settings are also available in the GUI. Except for view_center and hz, all are 32-bit integers. hz is a 32-bit floating point decimal and view_center is a 2-dimensional vector.
   */
  struct settings_t
  {
  ...
  /** \brief
   * The sim_t struct represents the whole simulation - the dominos, see-saw etc. It is like a global variable and an instance of this struct is referenced whenever something has to be done to this. 
   
   It has 2 variables :
   -# const char* name : this represents the name of the simulation
   -# sim_create_fcn *create_fcn : this creates the simulation. It is of typedef base_sim_t
   */
  struct sim_t
  {
    const char *name;
  ...
  /** \brief
   * The contact_point_t struct is called whenever there is a collision between 2 objects and this represents a *point of contact*. 
   
   The variables used to define this are:
   -# fixtureA, fixtureB : These are the 2 objects involved in the collision
   -# normal : The vector normal to the point of contact. It is of type b2Vec2 - a 2D vector
   -# position : The position where the objects have collided. This is also a 2D vector
   -# state : This depicts the state of points in contact
   */
  ...
  /** \brief
   * The base_sim_t class actually holds all the information of the simulation in the world. It has various functions for capturing keyboard and mouse actions. It also contains the title of the simulation, functions to detect when a joint has been destroyed and other callbacks for derived classes.
   */
  ...
  public:
    ...
    // These callbacks are made whenever two objects come in contact with each other
  ...
  //!How are protected members different from private members of a class in C++ ?
  protected:
